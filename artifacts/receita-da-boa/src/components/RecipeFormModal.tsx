  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (recipeToEdit) {
      update({ id: recipeToEdit.id, data: payload }, {
        ...
        onError: (err) => toast({ title: "Erro", description: err.error || "Falha ao atualizar", variant: "destructive" })
      })
    } else {
      create({ data: payload }, {
        onSuccess: () => {
        ...
        onError: (err) => toast({ title: "Erro", description: err.error || "Falha ao criar", variant: "destructive" })
      })
    }
  }
      ...
      <div className="p-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Compartilhe seus segredos culinários com o mundo.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold mb-2">Título da Receita <span className="text-destructive">*</span></label>
            ...
            <div>
              <label className="block text-sm font-semibold mb-2">Breve Descrição <span className="text-destructive">*</span></label>
              <Textarea required name="descricao" value={formData.descricao} onChange={handleChange} className="h-[216px]" placeholder="Conte um pouco sobre a receita..." />
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-semibold mb-2">Ingredientes <span className="text-destructive">*</span></label>
              ...
              <p className="text-xs text-muted-foreground mt-2">Coloque um ingrediente por linha.</p>
            </div>
            <div>
              <label className="block text-sm font-semibold mb-2">Modo de Preparo <span className="text-destructive">*</span></label>
              <Textarea required name="instrucoes" value={formData.instrucoes} onChange={handleChange} placeholder="1. Misture tudo&#10;2. Asse por 40 min&#10;..." className="min-h-[200px]" />
              <p className="text-xs text-muted-foreground mt-2">Coloque um passo por linha.</p>
            </div>
          </div>
