...
import { ChefHat, Search, LayoutGrid, Heart, Flame, Users, ShieldAlert, Plus, LogIn, LogOut, X, Menu } from "lucide-react"
...
export function Sidebar(props: Props) {
  const { user, categories, search, setSearch, activeFeed, setActiveFeed, categoriaId, setCategoriaId, onNewRecipe, onNewCategory } = props
  ...
  const sidebarBody = (
    <div className="relative p-6 md:p-8 flex flex-col h-full z-10">
      <Link href="/" onClick={close} className="flex items-center gap-3 mb-8 md:mb-10 hover:opacity-90 transition-opacity">
      ...
      <div className="space-y-8 flex-1 overflow-y-auto pr-2 pb-6 -mr-2">
        {/* Action Button */}
        {user && (
          <Button onClick={() => { onNewRecipe(); close(); }} className="w-full shadow-lg shadow-primary/20" size="lg">
        ...
        {user && (
          <div>
            ...
            <div className="space-y-1">
              <Link href={`/usuario/${user.id}`} onClick={close}>
              ...
              {user.papel === 'adm' && (
                <Link href="/moderacao" onClick={close}>
        ...
        <div>
          <div className="flex items-center justify-between mb-3 px-2">
            ...
            {user?.papel === 'adm' && (
              <button onClick={() => { onNewCategory(); close(); }} className="text-primary hover:text-primary/80 transition-colors">
          ...
          <div className="space-y-1">
            ...
            {categories?.map(cat => (
              <Link href="/" key={cat.id} onClick={close}>
                <button
  ...
  return (
    <>
      {/* Top bar - mobile only */}
      <div className="md:hidden flex items-center justify-between px-4 h-14 border-b border-border/60 bg-card/80 backdrop-blur-xl sticky top-0 z-30 flex-shrink-0">
        <Link href="/" className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary to-accent text-primary-foreground flex items-center justify-center shadow-md shadow-primary/30">
            <ChefHat className="w-5 h-5" />
          </div>
          <span className="font-display font-bold text-lg text-foreground leading-none">Receita da Boa</span>
        </Link>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-40 flex">
          ...
          <aside className="relative w-[85vw] max-w-sm bg-card border-r border-border/60 shadow-2xl flex flex-col">
            <button
              onClick={close}
              className="absolute top-3 right-3 p-2 rounded-xl hover:bg-muted text-muted-foreground z-20"
